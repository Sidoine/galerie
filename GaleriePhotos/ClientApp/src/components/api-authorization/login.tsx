...
export const Login = observer(({ action }: LoginProps) => {
  ...
  const store = useLocalStore(() => ({
    ...
    processLoginCallback: async () => {
      ...
      switch (result.status) {
        ...
        case "success":
          await store.navigateToReturnUrl(store.getReturnUrl(result.state));
          break;
        case "fail":
          store.message = result.message;
          break;
    ...
    redirectToProfile() {
      store.redirectToApiAuthorizationPath(ApplicationPaths.IdentityManagePath);
    },
